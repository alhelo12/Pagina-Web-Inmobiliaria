    ...
    <div class="list">
      <div v-for="p in items" :key="p.id" class="row">
        ...
        <div class="right">
          ...
          <small :class="p.status">{{ statusLabel[p.status] || p.status }}</small>
        </div>
      </div>
    </div>
  </article>
...
const fallback = FALLBACK_PROPERTY_IMAGE
...
.row { display: flex; align-items: center; gap: 12px; border: 1px solid var(--color-line); border-radius: 10px; padding: 10px 12px; background: #fff; overflow: hidden; }
...
.row strong { display: block; color: var(--color-navy); font-size: 14px; overflow: hidden; text-overflow: ellipsis; }
.row p { margin: 4px 0 0; color: var(--color-muted); font-size: 12px; }
.right { text-align: right; flex-shrink: 0; min-width: 0; }
